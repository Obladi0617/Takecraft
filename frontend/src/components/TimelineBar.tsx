...
export default function TimelineBar() {
  ...
  const render = useMutation({
    ...
    onSuccess: (result) => {
      setRenderUrl(`${API_BASE}${result.url}`)
    ...
    <div className="timeline-bar">
      <div className="timeline-toolbar">
        ...
        <span className="muted">
          {clips.length} 个{clips.some(c => c.asset_id) ? '场景' : '片段'} · 总时长 {total.toFixed(1)}s
        </span>
        ...
        {clips.map((clip, i) => {
          ...
          return (
            <div
              ...
              title={clip.shot_title ?? clip.shot_id ?? '场景视频'}
            >
              <span className="clip-index">{String(i + 1).padStart(2, '0')}</span>
              <span className="clip-title">
                {clip.shot_title ?? clip.shot_id ?? '片段'}
              </span>
              {clip.take_id && <span className="clip-take">{clip.take_id}</span>}
